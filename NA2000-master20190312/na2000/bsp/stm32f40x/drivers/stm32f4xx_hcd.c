 ...
 * @return the error code, RT_EOK on successfully.
 */
static rt_err_t susb_alloc_pipe(upipe_t* pipe, struct uintf* ifinst, uep_desc_t ep,
                                func_callback callback)
{
    ...
    USBH_Open_Channel(&USB_OTG_Core, channel, ifinst->device->address,
...
void rt_hw_susb_init(void)
{
    ...
    rt_device_register(&susb_hcd.parent, "usbh", 0);
		rt_usb_host_init();
}
