...
static void fsrw1_thread_entry(void* parameter)
{
    ...
    while(round<30)
    {
        if( stop_flag )
        {
            rt_kprintf("thread fsrw1 error,thread fsrw1 quit!\r\n");
        ...
        for(index=0; index<wr_num; index++)
        {
            length = write(fd, write_data1, fsrw1_data_len);		
            if (length != fsrw1_data_len)
            {
							rt_kprintf("fsrw1 write data failed:errno:%d  index = %d \n  ",rt_get_errno(),index);
                close(fd);
                fsrw1_thread = RT_NULL;
                stop_flag = 1;
                return;
        ...
        for(index=0; index<wr_num; index++)
        {
            ...
            if (length != fsrw1_data_len)
            {
                rt_kprintf("fsrw1 read file failed length = %d\r\n",length);
                close(fd);
                stop_flag = 1;
                fsrw1_thread = RT_NULL;
                return;
...
#define new_name2                   "/dev/test.c"
#define fsrw2_fn                   "/dev/tt.c"
#define fsrw2_data_len             120              /* Less than 256 */
static void fsrw2_thread_entry(void* parameter)
{
    ...
    while(round<30)
    {
        ...
        for(index=0; index<wr_num; index++)
        {
            ...
            if (length != fsrw2_data_len)
            {
								rt_kprintf("fsrw2 read file failed:errno:%d\r\n",rt_get_errno());
                close(fd);
                stop_flag = 1;
                fsrw2_thread = RT_NULL;
                return;
