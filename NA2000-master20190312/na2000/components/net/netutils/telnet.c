...
#define RT_SUPPORT_VERIFY_USERNAME_PASSWORD
#define RT_DEFAULT_USERNAME			"root"
#define RT_DEFAULT_PASSWORD			"naplc12345"
...
static char	*old_console_name = RT_NULL;
...
{
    /* set console */
    rt_console_set_device(old_console_name);
    /* set finsh device */
    finsh_set_device(old_console_name);
...
#ifdef RT_SUPPORT_VERIFY_USERNAME_PASSWORD
static rt_err_t verify_username_password(rt_int32_t clientid)
{
	#define RECV_BUF_LEN 64
		rt_err_t ret = RT_EOK;
		char username[RECV_BUF_LEN] = "";
		char password[RECV_BUF_LEN] = "";
		char temp;
		rt_uint8_t index = 0;
		...
		{
			index = 0;
			send(clientid, "login:", sizeof("login:"), 0);
			while(recv(telnet->client_fd, &temp, 1, 0) > 0 && temp != '\n' && index < RECV_BUF_LEN)
			{
				username[index++] = temp;
			}
			username[index-1] = 0;
			...
			index = 0;
			send(telnet->client_fd, "Password:", sizeof("Password:"), 0);
			while(recv(telnet->client_fd, &temp, 1, 0) > 0 && temp != '\n' && index < RECV_BUF_LEN)
			{
				password[index++] = temp;
				if(temp != '\r')
					send(clientid, "\b*", strlen("\b*"), 0);
			}
			password[index-1] = 0;

			if(	rt_strcmp(username,RT_DEFAULT_USERNAME) == 0 && rt_strcmp(password,RT_DEFAULT_PASSWORD) == 0)
			{
				break;
			}
		}
...
#define RECV_BUF_LEN 64
    ...
    while (1)
    {
        ...
        if ((telnet->client_fd = accept(telnet->server_fd, (struct sockaddr * )&addr, &addr_size)) == -1)
        {
			...
			#ifdef RT_SUPPORT_VERIFY_USERNAME_PASSWORD
				if(verify_username_password(telnet->client_fd) != RT_EOK)
					continue;
			#endif
        ...
        rt_kprintf("new telnet client(%s:%d) connection, switch console to telnet...\n", inet_ntoa(addr.sin_addr), addr.sin_port);
				...
				{
					/* set console */
					rt_device_t rdt = rt_console_set_device("telnet");
				 /* set finsh device */
					finsh_set_device("telnet");
					old_console_name = rdt->parent.name;
				}
...
void telnet_srv(void)
{
    ...
    if (telnet == RT_NULL)
    {
        ...
        tid = rt_thread_create("telnet", telnet_thread, RT_NULL, 1024, 21, 20);
        if (tid != RT_NULL)
