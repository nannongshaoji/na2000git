...
static rt_err_t rt_serial_rx_indicate(rt_device_t dev, rt_size_t size)
{
	serial_device* serial_dev = RT_NULL;
	RT_ASSERT(dev != RT_NULL);

	serial_dev = fd_get_serial_device(dev);

	rt_sem_release(&serial_dev->rx_sem);
                               ...
                               void                    *data)
{
    ...
    device->rx_indicate = rt_serial_rx_indicate;
